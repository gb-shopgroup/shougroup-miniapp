...
<view class="card">
	...
	<view class="card-header" style="display: flex; justify-content: space-between;">
		...
		<view class="btn btn-orange btn-small" @click="addSpec">添加包装</view>
	</view>
	...
	<view class="spec-list">
		<view class="spec-item" v-for="(item, index) in packList" :key="index">
			...
			<view style="display: flex; flex-direction: row-reverse; gap: 15rpx;">
				<view class="btn btn-danger btn-small" @click="removeSpec(item.id)">删除</view>
				<view class="btn btn-primary btn-small" @click="editSpec(item)">修改</view>
			</view>
		</view>
	</view>
	</view>
...
<AddPack ref="addPackRef" @childEvent="initGoodsPackList"  />
</view>
</template>
...
import { 
	getLeaderGoodsPackageList, 
	addLeaderGoodsPackageInfo, 
	editLeaderGoodsPackageInfo,
	removeLeaderGoodsPackageInfo } from "@/api/leader.js"
export default {
	data() {
		return {
			// 商品id
	...
	components: { LeaderHeader, AddPack },
	onLoad(options) {
		if(options.id){
	...
	methods: {
		...
		addSpec() {
			...
			this.$refs.addPackRef.show(null, this.goodsId)
		},
		// 修改包装
		editSpec(specInfo){
			
			this.$refs.addPackRef.show(specInfo, this.goodsId)
		},
		...
		async requestRemoveSpec(specId){
			...
			try {
				...
				const res = await removeLeaderGoodsPackageInfo(params)
				uni.showToast({ title: '删除成功', icon: 'success' })
				this.initGoodsPackList() // 刷新当前列表
			} catch (err) {
				console.log('请求删除包装失败：', err)
			}
		},
		// 返回商品列表
		goBack(){
