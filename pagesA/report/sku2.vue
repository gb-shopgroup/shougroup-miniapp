...
<!-- ********************************************************************** -->	
<view class="card" v-if="pointOrderGoodsList.length">
	...
	<view class="order-card" v-for="(item, index) in pointOrderGoodsList" :key="index">
		<view class="order-header">
			<text class="order-no">{{ item.name || '无规格' }}</text>
			<text class="order-num">{{ item.total }}</text>
		</view>
	</view>
	<view class="order-card" v-if="pointOrderGoodsList.length == 0">没有订单商品汇总数据</view>
    </view>
...
import { getSummaryPointOrderGoodsSkuInfo } from "@/api/leader.js"
export default {
	data() {
		return {
	...
	onLoad(options) {
		...
		if(options.pname){
			this.pointName = options.pname
		}

		// 指定提货点下的商品汇总
		this.initPointSummaryGoodsList()
	},
	methods: {
		// 汇总商品
		summaryDataList(){
		...
		// 指定提货点下的商品汇总
		async initPointSummaryGoodsList(){
			
			try {
				const params = { gid:this.goodsId, pid:this.pointId, start:this.start, end:this.end }
				const res = await getSummaryPointOrderGoodsSkuInfo(params)
				this.pointOrderGoodsList = res.data
			} catch (err) {
				console.log('指定提货点下的商品汇总失败：', err)
			}
		},
		// 日期选择器change事件
